...
class SKANDROID_ACTRL_Speedmatches extends OW_ApiActionController
{
    ...
    {
        return OW::getEventManager()->call('speedmatch.suggest_users', array(
            'userId' => $userId,
            'first' => $first,
            'count' => $count,
            'criteria' => $criteria,
    ...
    private function getDefaultFilter( $userId )
    {
        ...
        if ( !empty($data['match_sex']) )
        {
            ...
            if ( in_array($question->presentation, array(
    ...
    {
        return !empty($filter) && !empty($filter['googlemap_location'])
        && count(array_intersect(array_keys($filter['googlemap_location']), array('latitude', 'longitude', 'distance'))) === 3;
    }
