...
//            $count = MAILBOX_BOL_ConversationService::getInstance()->getUnreadMessageCount($userId);
    ...
    public function getAuthorizationActionStatus($groupName, $actionName = null, array $extra = null) {
        ...
        if (count($links)) {
            ...
            $error = array(
                ...
                'msg' => $lang->text(
                        'base', 'authorization_action_promotion', array('alternatives' => implode(' ' . $lang->text('base', 'or') . ' ', $links), 'action' => strtolower($actionLabel))
                )
            );
        }
    ...
    public function getAndroidAvailablePluginList() {
        return array('photo', 'mailbox', 'usearch', 'hotlist', 'bookmarks', 'ocsguests', 'membership', 'usercredits', 'googlelocation', 'base', 'skadate');
    }
