...
final class BOL_AvatarService {
     ...
     * @return array
     */
    public function getAvatarsUrlList(array $userIds, $size = 1, $flag) {
        if (empty($userIds)) {
            return array();
        }
        ...
        if (is_array($userIds)) {
            ...
            if ($flag == "1") {
                $prefix = OW::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . (self::AVATAR_BIG_PREFIX);
            } else {
    ...
    public function getDataForUserAvatars($userIdList, $src = true, $url = true, $dispName = true, $role = true, $flag) {
        if (!count($userIdList)) {
            return null;
        }
        $data = array();
        ...
        if ($src) {
            $srcArr = $this->getAvatarsUrlList($userIdList, '', $flag);
        }
